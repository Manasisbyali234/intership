...
const AdminNotes = () => {
  ...
  const fetchNotes = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/admin/notes');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = editingId ? `http://localhost:5001/api/admin/notes/${editingId}` : 'http://localhost:5001/api/admin/notes';
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem('adminToken');
    try {
      const response = await fetch(`http://localhost:5001/api/admin/notes/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
